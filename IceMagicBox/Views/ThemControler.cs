...
namespace IceMagicBox.Views
{
    public partial class ThemControler : UserControl
    {
        ...
        private void ThemControler_Load(object? sender, EventArgs e)
        {
            ...
            uploadDragger.Filter = "Image Files (*.jpg;*.jpeg;*.png;*.bmp;*.gif;*.tiff)|*.jpg;*.jpeg;*.png;*.bmp;*.gif;*.tiff";
            uploadDragger.HandDragFolder = true;//是否支持拖拽，默认为true
            uploadDragger.DragChanged += (s, e) => uploadDragger_Click(s, e);
            this.flowPanel1.Controls.Add(uploadDragger);
            AddImage3DControl();
        }

        private void ClearImage3DControl()
        {
            foreach (var item in this.flowPanel1.Controls)
            {
                if (item is Image3D image3d)
                {
                    this.flowPanel1.Controls.Remove(image3d);
                }
            }
        }

        private void AddImage3DControl()
        {

            var bgs = WallpaperChanger.GetWallPapers();
            foreach (var item in bgs)
            {
                ...
                image3D.Radius = 6;
                image3D.Image = item.Item1;
                image3D.Tag = item.Item2;
                image3D.Click += (s, e) =>
                {
        ...
        private void uploadDragger_Click(object sender, AntdUI.StringsEventArgs e)
        {

            WallpaperChanger.AddWallPapers(e.Value.ToList());
            ClearImage3DControl();
            AddImage3DControl();
        }
...
svg_delete ="<svg viewBox=\"0 0 1024 1024\"><path d=\"M106.666667 213.333333h810.666666v42.666667H106.666667z\" fill=\"#3D3D3D\" p-id=\"2642\"></path><path d=\"M640 128v42.666667h42.666667V128c0-23.573333-19.093333-42.666667-42.538667-42.666667H383.872A42.496 42.496 0 0 0 341.333333 128v42.666667h42.666667V128h256z\" fill=\"#3D3D3D\" p-id=\"2643\"></path><path d=\"M213.333333 896V256H170.666667v639.957333C170.666667 919.552 189.653333 938.666667 213.376 938.666667h597.248C834.218667 938.666667 853.333333 919.68 853.333333 895.957333V256h-42.666666v640H213.333333z\" fill=\"#3D3D3D\" p-id=\"2644\"></path><path d=\"M320 341.333333h42.666667v384h-42.666667zM490.666667 341.333333h42.666666v384h-42.666666zM661.333333 341.333333h42.666667v384h-42.666667z\" fill=\"#3D3D3D\" p-id=\"2645\"></path></svg>";
        ...
        new AntdUI.ContextMenuStripItem("删除", "Ctrl+D")
        {
            IconSvg = svg_delete
        },
        ...
        new AntdUI.ContextMenuStripItem("关于")
        {
            ...
            AntdUI.ContextMenuStrip.Config config = new AntdUI.ContextMenuStrip.Config(sender as Image3D, it =>
            {
                ...
                else if (it.Text.Contains("删除")) {

                    var image3d = (sender as Image3D);
                    this.flowPanel1.Controls.Remove(image3d);
                    image3d.Image.Dispose();
                    WallpaperChanger.RemoveWallPaper((sender as Image3D).Tag.ToString());
